...
const crypt = (key, buf, encrypt=true) => {
  ...
  while (length - cursor > 0) {
    ...
    if (encrypt) {
        bufs.push(crypto.privateEncrypt(key, buf.slice(cursor, size > max? cursor + max: length)))
    } else {
        bufs.push(crypto.publicDecrypt(key, buf.slice(cursor, size > max? cursor + max: length)))
    }
    cursor = cursor + max
  }
